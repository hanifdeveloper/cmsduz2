...
class Files {
	...
	public function getMimeTypes($file_type = ''){
		$mimes = array();
		if(empty($file_type)) return array();
		$exts = explode('|', $file_type);
		foreach($exts as $ext){array_push($mimes, $this->arrMimes[$ext]);}
		return implode(',', $mimes);		
	}
	
	public function upload($file, $opt){
		foreach($opt as $key => $value) $this->$key = $value;
		...
		if(!is_dir($dir_upload)){
			...
			if(isset($_SESSION[$this->project])){
